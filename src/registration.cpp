...
void ReplayCharDataToBeryl(struct mg_connection *c) {
  ...
  if (!storedStats.empty())
    SendPacketToBeryl(c, storedStats);

  for (auto &[slot, pkt] : storedSpells)
    SendPacketToBeryl(c, pkt);
  for (auto &[slot, pkt] : storedItems)
    SendPacketToBeryl(c, pkt);
  for (auto &[slot, pkt] : storedSkills)
    SendPacketToBeryl(c, pkt);
  for (auto &[slot, pkt] : storedEquipment)
    SendPacketToBeryl(c, pkt);
...
void TryRegister() {
  if (charRegistered || charName.empty())
    return;
  charRegistered = true;
