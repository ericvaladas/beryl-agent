...
void SendFileResponse(struct mg_connection* c, uint32_t requestId, uint8_t status,
                     const char* data = nullptr, size_t dataLen = 0) {
...
// ============================================================================
static void HandleMemoryRead(struct mg_connection* c, const BYTE* body, DWORD bodyLen) {
    if (bodyLen < 6) return;
    BYTE requestId = body[0];
    BYTE chainLength = body[1];
    DWORD expectedLen = 2 + (DWORD)chainLength * 4 + 4;
    if (bodyLen < expectedLen || chainLength == 0) return;
...
static void HandleMemoryWrite(struct mg_connection* c, const BYTE* body, DWORD bodyLen) {
    if (bodyLen < 6) return;
    BYTE requestId = body[0];
    BYTE chainLength = body[1];
    DWORD headerLen = 2 + (DWORD)chainLength * 4 + 4;
    if (bodyLen < headerLen || chainLength == 0) return;
