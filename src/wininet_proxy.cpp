...
void LoadRealWininet() {
    ...
    if (!hRealWininet) return;

    pInternetOpenA       = (decltype(&InternetOpenA))      GetProcAddress(hRealWininet, "InternetOpenA");
    pInternetOpenUrlA    = (decltype(&InternetOpenUrlA))   GetProcAddress(hRealWininet, "InternetOpenUrlA");
    pInternetSetCookieA  = (decltype(&InternetSetCookieA)) GetProcAddress(hRealWininet, "InternetSetCookieA");
    pInternetCloseHandle = (decltype(&InternetCloseHandle))GetProcAddress(hRealWininet, "InternetCloseHandle");
    pInternetReadFile    = (decltype(&InternetReadFile))   GetProcAddress(hRealWininet, "InternetReadFile");
}
...
BOOL WINAPI ProxyInternetReadFile(
    HINTERNET hFile, LPVOID lpBuffer, DWORD dwNumberOfBytesToRead,
