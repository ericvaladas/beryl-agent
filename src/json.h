...
private:
  ...
  template <typename T> static std::string fmt_int(T v) {
    char buf[32];
...
public:
  ...
  bool operator==(unsigned int v) const {
    return m_type == t_number && to_ulong() == v;
  }
  ...
  bool operator==(const char *s) const {
    return m_type == t_string && m_str == s;
  }
  ...
  void erase(const std::string &key) {
    m_obj.erase(
        std::remove_if(
            m_obj.begin(),
            m_obj.end(),
            [&key](const std::pair<std::string, json> &p) {
              return p.first == key;
            }
        ),
        m_obj.end()
